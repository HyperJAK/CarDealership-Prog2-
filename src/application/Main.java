...
public class Main {
	public static void main(String[] args) {
		...
		String directory = System.getProperty("user.dir");// gets current user
															// directory that
															// the program runs
															// in
		...
		String EmployeeFilePath = directory + "\\DealerShip\\Employee.csv"; // more
																			// specific
																			// directory
																			// of
																			// CSV
		...
		do {
			...
			optionStage_1 = InputChecker.nbIntChecker(); // uses method of main
															// class to check if
															// optionStage_1 is
															// integer or not
						...
						switch (optionStage_2) {
							case 1 -> {
								do {
									brandAvailable = true;
									System.out.print(
											"Enter car brand (BMW, Ferrari, Nissan, Audi, Toyota): ");
									...
									switch (brand.toLowerCase()) {
										...
										case "ferrari" -> {
											...
											String color = ferrari
													.createColor();
											int nbOfDoors = ferrari
													.createNbOfDoors();

											ferrari = new Ferrari(brand, color,
													nbOfDoors);
										...
										case "nissan" -> {
											...
											int nbOfDoors = nissan
													.createNbOfDoors();

											nissan = new Nissan(brand, color,
													nbOfDoors);
										...
										case "toyota" -> {
											...
											int nbOfDoors = toyota
													.createNbOfDoors();

											toyota = new Toyota(brand, color,
													nbOfDoors);
							...
							case 4 -> {
								...
								if (Cars.totalCars != 0) {
									do {
										brand = Cars.verifyBrand(true);// if
																		// brand
																		// not
																		// found
																		// Cars.carNotFound
										...
										if (!Cars.carNotFound) { // If it found
																	// a brand
																	// match it
																	// enters in
																	// if
						...
						switch (optionStage_2) {
							case 1 -> {
								if (Cars.totalCars != 0) {
									...
									String ID = clt.createClientID(fName, lName,
											tel);

									clt = new Client(fName, lName, age, tel,
									...
									do {
										brand = Cars.verifyBrand(false);// if
																		// brand
																		// not
																		// found
																		// Cars.carNotFound
									...
									System.out.println(Transaction
											.receipt(brand, carID - 1));
									Client.People_Clients
											.get(Client.clientCounter - 1)
											.addCarsToClient(brand, carID,
													Client.clientCounter - 1);

									CsvReaderWriter.carsWriteCsv(CarFilePath);
									CsvReaderWriter
											.clientWriteCsv(ClientFilePath);
							...
							case 3 -> {
								...
								if (ID == -1) {
									...
									do {
										...
										System.out.println(Client.People_Clients
												.get(ID).toString(ID));
										System.out.println(
												"""
										...
										switch (answer) {
											case 1 -> {
												...
												CsvReaderWriter.clientWriteCsv(
														ClientFilePath);
												CsvReaderWriter.carsWriteCsv(
														CarFilePath);
											}
											case 2 -> {
						...
						switch (optionStage_2) {
							case 1 -> {
								if (Employee.employeeCounter < 10) {
									...
									String ID = emp.createEmployeeID(fName,
											lName, tel);

									emp = new Employee(fName, lName, age, tel,
											address, ID.toLowerCase());
									emp.addEmployee(emp);
									CsvReaderWriter
											.employeeWriteCsv(EmployeeFilePath);
								} else {
									System.out.println(
											"You have reached the maximum number of employees you can have.");
								}
